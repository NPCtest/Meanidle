...
export class HaikyuudleGame {
    ...
    revealAnswer() {
        ...
        const imgUrl = t.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(t.name)}&background=ff6f00&color=fff&size=180&rounded=false&bold=true`;
        this.answerBox.innerHTML = `
            <img src="${imgUrl}" class="answer-img">
            <div>
                <strong>${t.name}</strong><br>
                <small>${t.team} · ${t.position} · Year ${t.year}</small>
            </div>
        `;
    ...
    makeGuess() {
        ...
        const results = [
            ...
            guess.compareString('team', this.targetCharacter),
            guess.compareString('prefecture', this.targetCharacter),
            guess.compareString('position', this.targetCharacter),
            guess.compareNumber('year', this.targetCharacter),
            guess.compareNumber('number', this.targetCharacter),
